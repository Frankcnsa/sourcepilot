...
// 每日爆品推荐 - 使用get-goods-list + 按总销量排序
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`[每日爆品] pageId=${pageId}, pageSize=${pageSize}`);

    const { nonce, timer, signRan } = generateSign();
    
    // 使用get-goods-list，按总销量降序排序
    ...
    url.searchParams.set('sort', 'totalSales_desc'); // 按总销量降序
